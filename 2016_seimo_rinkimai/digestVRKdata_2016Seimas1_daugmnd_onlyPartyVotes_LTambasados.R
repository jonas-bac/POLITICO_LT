...
#  ONLY party level votes on embassy/consulate level
...
ls_files = list.files(director,
                      pattern = "2016_vrk_raw_data_LTambasados_onlyPartyVotes")


#####################   LIETUVOS AMBASADOS IR KONSULATAI   ########################

# load VRK data
a = readLines(paste(director,ls_files[1],sep="")) # no repeated grabs were necessary
...
till = grep("^Pastaba. Duomenys išrikiuoti pagal",a) - 2  # note, i changed here, since some embassies have no data
...
if (length(table(n_part))>1) warning("nonsense from-till values in map. embassy will be deleted")
...
unique(master$apylinke)
## assign short merging index for apylinke
master$apygapyl = paste("Diplomatinė",master$apylinke,sep="___")        #master$apygarda
...
# doublecheck the fullness of the grabbed data (should be 49 as for Oct15)
...
length(unique(master$tst)) # should be 49*14 = 686  !!! ***
...
transl4 = read.table("~/Dropbox/GIT/POLITIKA_LT/2016_seimo_rinkimai/konsulatu_zodynas.txt",h=T,sep="\t",stringsAsFactors = F)
if (!all(transl4$apygapyl_hash %in% dat$apygapyl_hash)) warning("nesutampa hashai konsulatu!")
if (!all(dat$apygapyl_hash %in% transl4$apygapyl_hash)) warning("nesutampa hashai konsulatu!")
dat = merge(dat,transl4,by="apygapyl_hash",all=T)
dat = dat[,which(colnames(dat)!="lokacijos_fragmentas")]


# cleanup and save
rm(list = ls()[which(ls()!="dat")])
save(list="dat",file="~/Dropbox/GIT/POLITIKA_LT/2016_seimo_rinkimai/VRK_2016Seim1_daugmand_onlyPartyVotes_ambasad.RData")

# for further analysis load:
load("~/Dropbox/GIT/POLITIKA_LT/2016_seimo_rinkimai/VRK_2016Seim1_daugmand_onlyPartyVotes_ambasad.RData")
# or
