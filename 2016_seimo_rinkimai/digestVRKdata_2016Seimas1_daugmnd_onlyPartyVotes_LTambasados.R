...
master$apygapyl = paste("Diplomatinė",master$apylinke,sep="___")        
...
# doublecheck the fullness of the grabbed data (should be 51 , but only 49 with data..?)
...
length(unique(master$tst)) # should be 51*14 = 714  !!! ***
...
df = as.data.frame(df)
hist(df$apyl_total); sort(df$apyl_total)[1:10] # note, two embassies have zero votes
...
if (!all(dat$apygapyl_hash %in% transl4$apygapyl_hash)) warning("nesutampa hashai konsulatu!") # ok if warning: transl4 has 49
dat = merge(dat,transl4,by="apygapyl_hash",all=F)  # CAREFUL with "F"! should be 686 ***
